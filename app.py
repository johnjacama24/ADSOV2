...
if st.button("Realizar predicción"):
    try:
        columnas_modelo = normalizar_columnas(df.drop("ESTADO APRENDIZ", axis=1).columns)
        ...
        if faltantes:
            ...
            predicción_codificada = modelo.predict(entrada_modelo)[0]
            predicción_original = diccionario_inverso.get(predicción_codificada, f"Desconocido ({predicción_codificada})")

            # Mostrar resultado
            st.subheader("Resultado de la predicción:")
            st.success(f"📊 Estado del aprendiz predicho: **{predicción_original}**")
